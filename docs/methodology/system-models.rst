...
.. raw:: html
   ...
   <span id="cut-off-and-consequential-modelling"></span>


This chapter explains how consequential markets identify suppliers responding
to a change in demand. The preceding sector chapters describe inventory
transformations; their applicability and supplier composition depend on the
selected system model. See :doc:`workflow` for the short system-model overview.
...
Identifying marginal suppliers
--------------------------------

Production trajectories over a selected observation interval inform the
marginal supplier mix. This differs from weighting a market by its production
shares in a single year. Demand duration, foresight and replacement assumptions
determine how those trajectories are interpreted, as described below.
...
Setting both ``range time`` and ``duration`` to zero selects a zero-window
...
<https://github.com/polca/premise/blob/76dbf845ef73bb765024dda1143960a24964a5fe/premise/data/consequential/leadtimes.yaml>`_.
...
<https://github.com/polca/premise/blob/76dbf845ef73bb765024dda1143960a24964a5fe/premise/data/consequential/lifetimes.yaml>`_.
...
* ``5`` -- production-volume weighting. It selects suppliers according
  ...
  a production-level alternative to change-based selection; methods 0 to 4
...
The general marginal-supplier method is applied where the scenario provides
the required production data. Electricity markets and fuel markets retain
their sector-specific boundaries, exclusions and compatibility rules.

See :doc:`electricity/markets` for voltage layers and supply composition, and
:doc:`fuels/markets` for fuel-family rules, calorific values and downstream
emissions. These pages own the detailed sector rules; the parameters below
configure the common consequential calculation.
...
<https://github.com/polca/premise/blob/76dbf845ef73bb765024dda1143960a24964a5fe/premise/data/consequential/blacklist.yaml>`_
