...
def car(energy=0.8, fuel="natural gas", unit="kilogram", production=1):
    lhv = {"natural gas": 47.5 if unit == "kilogram" else 36,
           "diesel": 43, "petrol": 42.6}[fuel]
    factor = {"natural gas": .06, "diesel": .0732, "petrol": .0737}[fuel]
    powertrain = {"natural gas": "compressed gas", "diesel": "diesel", "petrol": "gasoline"}[fuel]
    return {"name": f"transport, passenger car, {powertrain}, Small, EURO-6",
            "reference product": "transport, passenger car, EURO-6", "location": "NEU",
            "unit": "kilometer", "exchanges": [
                {"name": "car service", "type": "production", "amount": production},
                {"name": f"market for {fuel}", "unit": unit, "type": "technosphere", "amount": energy/lhv*production},
                {"name": "vehicle manufacture", "type": "technosphere", "unit": "unit", "amount": .001},
                {"name": "Carbon dioxide, fossil", "categories": ("air",), "type": "biosphere", "amount": energy*factor*production},
                {"name": "Particulates, tyre wear", "categories": ("air",), "type": "biosphere", "amount": .001},
                {"name": "Water", "categories": ("natural resource",), "type": "biosphere", "amount": .002},
            ]}
...
def validate(ds):
    ...
    issues = []
    validator.log_issue = lambda ds, reason, message, issue_type="minor": issues.append((reason, message, issue_type))
    validator.check_car_fuel_energy(ds)
    return issues


@pytest.mark.parametrize("fuel,unit", [("diesel", "kilogram"), ("petrol", "kilogram"),
                                       ("natural gas", "kilogram"), ("natural gas", "cubic meter")])
def test_floor_and_validation(fuel, unit):
    ds = car(fuel=fuel, unit=unit)
    before = deepcopy(ds)
    event = apply_floor(ds)
    assert fuel_balance(ds)[0] == pytest.approx(.852)
    assert event["floor correction factor"] == pytest.approx(.852/.8)
    assert validate(ds) == []
...
@pytest.mark.parametrize("energy", [.852, 1., 2.])
...
def test_multiple_fuels_and_nonunit_production():
    ds = car(.4, production=2)
    extra = car(.4, fuel="diesel", production=2)
    ds["exchanges"].extend([extra["exchanges"][1], extra["exchanges"][3]])
    before_ratio = ds["exchanges"][1]["amount"] / ds["exchanges"][-2]["amount"]
    assert fuel_balance(ds)[0] == pytest.approx(.8)
    apply_floor(ds)
    assert fuel_balance(ds)[0] == pytest.approx(.852)
    assert ds["exchanges"][1]["amount"] / ds["exchanges"][-2]["amount"] == pytest.approx(before_ratio)
    assert validate(ds) == []


@pytest.mark.parametrize("name", ["transport, passenger car, plugin gasoline hybrid, Small",
...
def test_configurable_and_disabled_floor():
    ds = car()
    config = {"enabled": True, "minimum_mj_per_vehicle_km": .852,
              "overrides": {"compressed gas": {"small": .9}}}
    assert minimum_energy(ds, config) == .9
    apply_floor(ds, config)
    assert fuel_balance(ds)[0] == pytest.approx(.9)
...
def test_uncertainty_and_carbon_split_preserved():
    ds = car()
    fuel = ds["exchanges"][1]
    fuel.update({"uncertainty type": 2, "loc": np.log(fuel["amount"]), "scale": .2})
    bio = deepcopy(ds["exchanges"][3])
    bio.update(name="Carbon dioxide, non-fossil", amount=.01)
...
def test_recorded_neu_2050_regression():
    ...
    ds["exchanges"][3]["amount"] = .04700672745633079
...
@pytest.mark.parametrize("fuel,alias", [("natural gas", "natural gas"), ("petrol", "gasoline")])
def test_transformation_applies_floor_after_iam_and_logs_it(fuel, alias):
    ...
    tr.iam_data = SimpleNamespace(passenger_car_efficiencies=xr.DataArray([1.], coords={"variables": ["gas car"]}, dims=["variables"]))
    tr.find_iam_efficiency_change = lambda **kwargs: 1.25
    tr.write_log = lambda ds: None
    tr.adjust_transport_efficiency(ds)
    assert fuel_balance(ds)[0] == pytest.approx(.852)
    assert ds["log parameters"]["car energy floor"]["source energy MJ/km"] == pytest.approx(1.)
    assert validate(ds) == []
