...
def _pipeline_conversion_inputs(activity, demand_amount):
    if normalized(activity.get("name")) != "hydrogen supply, distributed by pipeline":
        return []
    scale = demand_amount / _reference_output_amount(activity)
    inputs = []
    for exchange in activity.technosphere():
        provider = exchange.input
        exchange_name = normalized(exchange.get("name") or provider.get("name"))
        if exchange_name in CONVERSION_NAMES:
            inputs.append(
                {
                    "provider": provider,
                    "demand amount": scale * float(exchange.get("amount", 0.0)),
...
def select_markets(database):
    ...
    generic_matches = exact_matches(cfg.GENERIC_MARKET_NAME, cfg.TARGET_LOCATION)
    ...
    fallback_locations = sorted(set(sector_locations.values()) - {cfg.TARGET_LOCATION})
    ...
    sector_order = [label for label in cfg.EXPECTED_SECTOR_MARKETS if label in selected]
...
def select_methods():
    ...
    missing_ced = [method for method in cfg.CED_METHODS if method not in bd.methods]
    if missing_ced:
        raise LookupError(f"Required CED methods are unavailable: {missing_ced}")
...
def calculate_lcia(selected, market_order, lcia_methods, method_units):
    ...
    check = scores_df[scores_df["method"].isin([cfg.PREMISE_GWP_METHOD])].merge(
        reconstructed, on=["market", "method"], validate="one_to_one"
    )
    differences = (check["score per kg H2"] - check["reconstructed score"]).abs()
...
def analyze_hydrogen_life_cycle_stages(
    ...
    for market_label in market_order:
        ...
        for branch in branches:
            ...
            stage, substage, short_name, rule = classify_market_branch(provider)
            branch_score = scorer.score(provider, demand_amount)
            direct_rows = _direct_biosphere_rows(provider, demand_amount, cf_lookup)
            leakage_score = sum(
                row["score"]
            ...
            if stage == "Production":
                            ...
                            _score_fields(
                                {
                                    ...
                                    "score": sum(row["score"] for row in leakage_rows),
                                },
                                market_total,
                            )
                        )
                ...
                process_nonleakage = branch_score - leakage_score - conversion_score
                distribution_nonleakage += process_nonleakage + conversion_score
                ...
                for leakage_type in ("Hydrogen leakage", "Ammonia leakage"):
                    ...
                    if leakage:
                        ...
                        physical_amount = sum(row["physical amount"] for row in leakage)
    ...
    production_groups_df["share of market (%)"] = np.where(
        ...
        100.0 * production_groups_df["score"] / production_groups_df["score per kg H2"],
        np.nan,
    )
    reconciliation_rows = []
    for market_label in market_order:
        ...
        reconciliation_rows.append(
            {
                ...
                "Layer 2 reconstructed score": production_sum + distribution_sum,
                "Layer 2 difference": production_sum + distribution_sum - total,
            }
        )
...
def _spider_ratios(scores_df, ef31_methods, market_order):
    ...
    ef_scores = (
        ...
        .pivot(index="impact category", columns="market", values="score per kg H2")
...
def _hotspot_tables(selected, market_order, ef31_methods, scores_df, method_units):
    candidates = [*ef31_methods, cfg.PREMISE_GWP_METHOD]
    methods_by_category = {}
    for category in cfg.SPECIFIC_IMPACT_CATEGORY_ORDER:
        matches = [
            method for method in candidates if impact_category_label(method) == category
        ]
        if len(matches) != 1:
            raise LookupError(
    ...
    groups = groups.merge(totals, on=["market", "method"], validate="many_to_one")
...
def _regional_steel_analysis(database, method_units):
    activities = sorted(
        [
            ...
            and activity.get("reference product") == cfg.REFERENCE_PRODUCT
            and activity.get("unit") == cfg.UNIT
            and str(activity.get("location", "")).upper() != cfg.EXCLUDED_STEEL_LOCATION
        ],
    ...
    if duplicates:
        raise LookupError("Duplicate steel-market locations: " + ", ".join(duplicates))
    ...
    records = []
    for region, activity in selected.items():
        lca = bc.LCA({activity: cfg.FUNCTIONAL_UNIT_KG}, cfg.PREMISE_GWP_METHOD)
        lca.lci()
        lca.lcia()
        records.append(
            {
                ...
                "market name": activity.get("name"),
                "location": activity.get("location"),
                "impact category": impact_category_label(cfg.PREMISE_GWP_METHOD),
...
def export_tables(results: AnalysisResults) -> dict[str, Path]:
    ...
    if len(set(paths.values())) != len(paths):
        raise AssertionError("Two result tables resolve to the same export path.")
...
def run_analysis(
    ...
    *,
    ...
    selected, market_order, sector_order, selection_df = select_markets(database)
    ...
    tables = {
        ...
        "EF 3.1 spider ratios": spider_df.rename_axis(columns="market").reset_index(),
        ...
        tables.update(
            {
                ...
                "Steel-region distribution": steel_stage["distribution processes"],
