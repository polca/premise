...
def setup_database(project_name):
    ...
    if db_name not in bd.databases:
        ...
        if delay_seconds > 0:
            log(f"Waiting {delay_seconds}s before ecoinvent download to avoid rate limiting...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    log(f"Import failed (attempt {attempt + 1}/{max_retries}): {str(e)}")
                    log(f"Retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    log(f"Failed to import ecoinvent after {max_retries} attempts", level="ERROR")
                    raise
    else:
...
def create_datapackage_chunk(chunk_scenarios, chunk_num, model, project_name, model_config):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "cached_files" in str(e) and attempt < max_retries - 1:
                    ...
                    retry_delay = ((int(job_id) % 7) + (chunk_num_val % 5) + attempt) * 20 + 30

                    log(f"  Cache file not ready, retrying in {retry_delay}s (attempt {attempt + 1}/{max_retries})...")
...
def main():
    ...
    if len(sys.argv) != 5:
        log("ERROR: Usage: python MIC-datapackages-chunk.py <model> <chunk_num> <total_chunks> <chunk_size>")
        return 1
