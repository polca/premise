...
CONFIG = {
    "local_iam_models": {
        "image": {
            "folder": "image",  # relative path to IAM files
            "enabled": True
        },
        "remind": {
            "folder": "remind",
...
def get_working_pathways(model):
    ...
    is_local = (
            model in CONFIG["local_iam_models"]
            and CONFIG["local_iam_models"][model].get("enabled", False)
    )
    ...
    if is_local:
        ...
        if not iam_files:
            ...
            return {"pathways": [], "model": model, "mode": "local", "folder": folder_name}
        ...
        for chunk_num in range(1, num_chunks + 1):
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    is_local = (
                            model in CONFIG["local_iam_models"] and
                            CONFIG["local_iam_models"][model].get("enabled", False)
                    )
