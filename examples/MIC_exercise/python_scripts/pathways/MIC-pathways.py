...
def get_geography_mapping(datapackage_path):
    ...
    try:
        with zipfile.ZipFile(datapackage_path, 'r') as zip_file:
            ...
            scenario_data = zip_file.read('scenario_data/scenario_data.csv').decode('utf-8')
            ...
            geo_mapping_file = f"/data/user/hahnme_a/MIC_exercise/geo_mapping_{model}.yaml"

            if not Path(geo_mapping_file).exists():
                raise FileNotFoundError(f"Geography mapping not found: {geo_mapping_file}")
...
def analyze_datapackage(datapackage_path, year, output_suffix=""):
    ...
    # Define variables
    variables = [key for key, value in p.mapping.items() if key.startswith(('FE - final energy', 'SE - cdr'))]
    ...
    variables = [var for var in variables if not var.startswith('FE - final energy - CDR')]
    ...
    print(f"Calculating for {len(scenarios)} scenarios, year {year}, {len(methods)} methods")
...
def main():
    parser = argparse.ArgumentParser(description='Run pathways analysis on MIC datapackage')
    parser.add_argument('--datapackage', required=True, help='Path to datapackage ZIP file')
