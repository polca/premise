...
def consequential_method(
    ...
    CRR=0,
    measurement=0,
    weighted_slope_start=0.75,
    weighted_slope_end=1,
):
    """
    ...
    for region in data.coords["region"].values:
        ...
        avg_cap_repl_rate = (
            -1
            / avg_lifetime
            * data_full.sel(region=region, year=avg_start).sum(dim="variables").values
        )
        # again was put in to deal with Nan values in data
        ...
        volume_change = (
            (
                data_full.sel(region=region, year=avg_end).sum(dim="variables")
            ...
            for variables in data.coords["variables"]:
                market_shares.loc[dict(region=region, variables=variables)] = (
                    data_full.sel(
                        region=region,
                        variables=variables,
                        year=end.sel(region=region, variables=variables),
                    ).values
                    - data_full.sel(
                        region=region,
                        variables=variables,
                        year=start.sel(region=region, variables=variables),
                    ).values
                ) / (
                    end.sel(region=region, variables=variables)
                    - start.sel(region=region, variables=variables)
                )

        if CRR == 0 and measurement == 1:
            for variables in data.coords["variables"]:
                a = data_full.sel(region=region, variables=variables).where(
                    data_full.sel(region=region, variables=variables).year
                    >= start.sel(region=region, variables=variables).values
                )
                b = a.where(
                    a.year <= end.sel(region=region, variables=variables).values
                )
                c = b.polyfit(dim="year", deg=1)
                market_shares.loc[
                    dict(region=region, variables=variables)
        ...
        if CRR == 0 and measurement == 2:
            for variables in data.coords["variables"]:
                ...
                n = (
                    end.sel(region=region, variables=variables).values
                    - start.sel(region=region, variables=variables).values
                )
                total_area = 0.5 * (
                    2 * c
        ...
        if CRR == 0 and measurement == 3:
            for variables in data.coords["variables"]:
                ...
                short_slope = (
                    data_full.sel(
                        region=region, variables=variables, year=short_slope_end
                    ).values
                    - data_full.sel(
                        region=region, variables=variables, year=short_slope_start
                    ).values
            ...
            for y in split_years:
                ...
                for variables in data.coords["variables"]:
                    market_shares_split.loc[
                        ...
                        data_full.sel(
                            region=region, variables=variables, year=y + 1
                        ).values
                        - data_full.sel(
                            region=region, variables=variables, year=y
                        ).values
                    )
        ...
        if CRR == 1 and measurement == 0:
            for variables in data.coords["variables"]:
                ...
                cap_repl_rate = (
                    -1
                    / lifetime.sel(variables=variables).values
                    * data_full.sel(
                        region=region,
                        variables=variables,
                        year=start.sel(region=region, variables=variables),
