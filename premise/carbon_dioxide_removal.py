...
class CarbonDioxideRemoval(BaseTransformation):
                    ...
                    for region, dataset in new_ds.items():
                        ...
                        if technology == "direct air capture" and not any(
                            ...
                            if self.iam_data.daccs_energy_use is not None:
                                if (
                                    self.iam_data.daccs_energy_use.sel(region=region)
                                    .interp(year=self.year)
                                    .sum()
                                    > 0
                                ):
                                    energy_dataset_name = "market for energy, for direct air capture and storage"

                        if technology == "enhanced rock weathering":
                            if self.iam_data.ewr_energy_use is not None:
                                if (
                                    self.iam_data.ewr_energy_use.sel(region=region)
                                    .interp(year=self.year)
                                    .sum()
                                    > 0
                                ):
                                    energy_dataset_name = "market for energy, for enhanced rock weathering"
    ...
    def create_cdr_markets(
        ...
        def generate_regional_markets(
            ...
            cdr_mix = dict(
                zip(
                    ...
                    self.iam_data.cdr_technology_mix.sel(region=region)
                    .interp(year=self.year)
                    .values,
                )
            )
        ...
        new_datasets = [
            ...
            if region != "World"
            and self.iam_data.cdr_technology_mix.sel(region=region)
            .interp(year=self.year)
            .sum()
            > 0
        ]
