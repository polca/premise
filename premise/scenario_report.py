...
def generate_summary_report(scenarios: IAMDataCollection, filename: Path) -> None:
    """
        ...
        for scenario_idx, scenario in enumerate(
            ...
            if (model, pathway) not in scenario_list:
                ...
                if "generation" in sector:
                    iam_data = scenarios.production_volumes.sel(scenario=scenario)
                    ...
                    if scenarios.trsp_cars is not None:
                        if scenario in scenarios.trsp_cars.scenario.values:
                            iam_data = scenarios.trsp_cars.sel(scenario=scenario).sum(
                    ...
                    if scenarios.trsp_buses is not None:
                        if scenario in scenarios.trsp_buses.scenario.values:
                            iam_data = scenarios.trsp_buses.sel(scenario=scenario).sum(
                                dim=["size", "construction_year"]
                            )
                            iam_data = iam_data.rename({"powertrain": "variables"}).T
                        else:
                    ...
                    if scenarios.trsp_trucks.sel(scenario=scenario) is not None:
                        if scenario in scenarios.trsp_trucks.scenario.values:
                            iam_data = scenarios.trsp_trucks.sel(scenario=scenario).sum(
                                dim=["size", "construction_year"]
                            )
                            iam_data = iam_data.rename({"powertrain": "variables"}).T
                        else:
                    ...
                    iam_data = scenarios.other_vars.sel(scenario=scenario)
                ...
                worksheet.cell(
                    column=col,
                    row=row,
                    value=f"{model.upper()} - {pathway.upper()}",
                )
                ...
                scenario_list.append((model, pathway))
