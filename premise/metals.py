...
class Metals:
    ...
    def fetch_proxies(self, name):
        ...
        d_map = {
            ...
            for d in ws.get_many(self.db, ws.equals("name", name))
        }
        ...
        for d in d_remind_to_eco:
            try:
                ...
                print(
                    "Multiple results for {} found for the REMIND region {}".format(
                        name, d
                    )
                )
        ...
        # Remove old datasets
        self.db = [act for act in self.db if act["name"] != name]
        ...
        for exc in ws.biosphere(
            ...
            try:
                remind_emission = self.iam_data.steel_emissions.loc[
                    dict(region=ds["location"], pollutant=remind_emission_label)
                ].values.item(0)
            except KeyError:
                # TODO: fix this.
                # GAINS does not have a 'World' region, hence we use China as a temporary fix
                remind_emission = self.iam_data.steel_emissions.loc[
                    dict(region="CHA", pollutant=remind_emission_label)
                ].values.item(0)
    ...
    def adjust_recycled_steel_share(self, dict_act):
        ...
        for d, act in dict_act.items():
            ...
            total_production_volume = self.steel_data.sel(
                region=remind_region, variables="Production|Industry|Steel"
            )
            primary_share = (
                self.steel_data.sel(
                    region=remind_region, variables="Production|Industry|Steel|Primary"
                )
    ...
    def generate_activities(self, industry_module_present=True):
        ...
        if industry_module_present:
            ...
            with open(
                DATA_DIR / "logs/log deleted steel datasets.csv", "w"
            ) as csv_file:
                writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
                writer.writerow(["dataset name", "reference product", "location"])

            with open(
                DATA_DIR / "logs/log created steel datasets.csv", "w"
            ) as csv_file:
                writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
                writer.writerow(["dataset name", "reference product", "location"])
            ...
            d_act_primary_steel = {
                mat: self.fetch_proxies(mat)
                for mat in self.material_map["steel, primary"]
            }
            d_act_secondary_steel = {
                mat: self.fetch_proxies(mat)
                for mat in self.material_map["steel, secondary"]
            }
            ...
            d_act_steel = {
                k: self.remove_exchanges(v, list_fuels) for k, v in d_act_steel.items()
            }

            # List final energy carriers used in steel production
            l_FE = [
                v.split("|")
                for v in self.steel_data.coords["variables"].values
                if "FE" in v and "steel" in v.lower() and "electricity" not in v.lower()
            ]
            ...
            list_second_fuels = [
                ...
                for _, g in itertools.groupby(
                    list_second_fuels, lambda x: x.split("|")[1]
                )
            ]
            ...
            for d in d_act_steel:
                ...
                for k in d_act_steel[d]:
                    ...
                    for count, fuel_type in enumerate(
                        ["|".join(y) for y in l_FE if "Primary" in y]
                    ):

                        # Amount of specific fuel, for a specific region
                        fuel_amount = self.steel_data.sel(
                            variables=fuel_type, region=k
                        ) * (
                            self.steel_data.sel(
                                variables=list_second_fuels[count], region=k
                            )
                            / self.steel_data.sel(
                                variables=list_second_fuels[count], region=k
                            ).sum(dim="variables")
                        ...
                        fuel_amount /= self.steel_data.sel(
                            region=k, variables="Production|Industry|Steel|Primary"
                        )
                        ...
                        for c, i in enumerate(fuel_amount):
                            if i > 0:
                                (
                                    ...
                                    list_second_fuels[count][c]
                                ].values()
                                fuel_lhv = self.fuels_lhv[fuel_name]
                                fuel_qty = i.values.item(0) / fuel_lhv
                                fuel_fossil_co2 += (
                                    fuel_qty
                                    * self.fuels_co2[fuel_name]["co2"]
                                    * (1 - self.fuels_co2[fuel_name]["bio_share"])
                                )
                                fuel_biogenic_co2 += (
                                    fuel_qty
                                    * self.fuels_co2[fuel_name]["co2"]
                                    * self.fuels_co2[fuel_name]["bio_share"]
                                )
                                ...
                                fuel_suppliers = self.get_shares_from_production_volume(
                                    self.get_suppliers_of_a_region(
                                        [k], [activity_name], fuel_ref_prod
                                    )
                                )
                                if len(fuel_suppliers) == 0:
                                    fuel_suppliers = (
                                        self.get_shares_from_production_volume(
                                            self.get_suppliers_of_a_region(
                                                ["World", "EUR"],
                                                [activity_name],
                                                fuel_ref_prod,
                                    ...
                                    new_exchanges.append(
                                        {
                                            "uncertainty type": 0,
                                            "loc": 1,
                                            "amount": fuel_suppliers[supplier]
                                            * fuel_qty,
                                            "type": "technosphere",
                                            "production volume": 1,
                                            "product": supplier[2],
                    ...
                    try:
                        fossil_co2_exc = [
                            e
                            for e in d_act_steel[d][k]["exchanges"]
                    ...
                    if d in self.material_map["steel, primary"]:
                        ...
                        electricity = (
                            (
                                self.steel_data.sel(
                                    region=k,
                                    variables="FE|Industry|Electricity|Steel|Primary",
                                ).values
                                / self.steel_data.sel(
                                    region=k,
                        ...
                        electricity = (
                            (
                                self.steel_data.sel(
                                    region=k,
                                    variables="FE|Industry|Electricity|Steel|Secondary",
                                ).values
                                / self.steel_data.sel(
                                    region=k,
                                    variables="Production|Industry|Steel|Secondary",
                                ).values
                            )
                            * 1000
                            / 3.6
                ...
                d_act_steel[d] = {
                    ...
                    for k, v in d_act_steel[d].items()
                }
            ...
            print(
                "Relink new steel production activities to specialty steel markets and other steel-consuming activities "
            )

            with open(
                DATA_DIR / "logs/log created steel datasets.csv", "a"
            ) as csv_file:
                writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
                ...
                # If it fails
                    ...
                    try:
                        ...
                        if ds["location"] in self.iam_data.regions:
                                ...
                                *[
                                    ...
                                    ws.either(
                                        *[
                                            ...
                                            for l in self.geo.iam_to_ecoinvent_location(
                                                ds["location"]
                                            )
                                        ]
                                    ),
                            ...
                            exc["location"] = new_supplier["location"]
                            print(
                                f"found it! {new_supplier['name']}, {new_supplier['location']}"
                            )
                            ...
                            try:
                                ...
                                ds["exchanges"] = [
                                    ...
                                    if e["name"] != "market for steel, low-alloyed"
                                ]
