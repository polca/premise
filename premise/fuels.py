...
class Fuels(BaseTransformation):
        ...
        for hydrogen_type, hydrogen_activity in hydrogen_sources.items():
            ...
            for region, dataset in new_ds.items():
                ...
                if hydrogen_type == "from electrolysis":
                    ...
                    electricity_consumption = adjust_electrolysis_electricity_requirement(
                        self.year
                    )

                    # remove electricity inputs
                    dataset["exchanges"] = [
                        exc for exc in dataset["exchanges"] if exc["unit"] != "kilowatt hour"
                    ]
    ...
    def add_hydrogen_input_and_losses(
        ...
        if "log parameters" not in dataset:
            dataset["log parameters"] = {}
        dataset["log parameters"].update({"hydrogen distribution losses": total_loss - 1})
    ...
    def add_hydrogenation_energy(
        ...
        if "log parameters" not in dataset:
            dataset["log parameters"] = {}
        dataset["log parameters"].update({"electricity for hydrogen compression after dehydrogenation": electricity_comp})
    ...
    def add_compression_electricity(
        ...
        if "log parameters" not in dataset:
            dataset["log parameters"] = {}
        dataset["log parameters"].update({"electricity for hydrogen compression": electricity_comp})
    ...
    def add_pre_cooling_electricit(self, dataset: dict, region: str) -> dict:
        ...
        if "log parameters" not in dataset:
            dataset["log parameters"] = {}
        dataset["log parameters"].update({"electricity for hydrogen pre-cooling": el_pre_cooling})
                        ...
                        dataset["log parameters"].update(
                            {
                                ...
                                "final biomass per kg biofuel": e["amount"] * scaling_factor,
                            }
                        )
    ...
    def fetch_fuel_share(
            self, fuel: str, relevant_fuel_types: List[str], region: str
    ) -> float:
        """
    ...
    def relink_activities_to_new_markets(self):
        ...
        for dataset in ws.get_many(
                self.database, ws.doesnt_contain_any(
                    "name", [consumer for consumer in fuel_consumers]
                )
        ):
            ...
            if any(
                ...
                for _, activity in fuel_markets.items():
                    if activity["name"] in fuel_consumers:
                        ...
                        dataset["exchanges"] = [
                            exc for exc in dataset["exchanges"] if exc not in excs
                        ]
    ...
    def generate_world_fuel_market(
            self, dataset: dict, d_act: dict, prod_vars: list
    ) -> tuple:
        """
        ...
        total_prod_vol = (
            self.iam_data.production_volumes.sel(region="World", variables=prod_vars)
            .interp(year=self.year)
            .sum(dim="variables")
            .values.item(0)
        )
        ...
        for r in d_act.keys():
            ...
            region_prod = (
                self.iam_data.production_volumes.sel(region=r, variables=prod_vars)
                .interp(year=self.year)
                .sum(dim="variables")
                .values.item(0)
            )
    ...
    def calculate_fuel_properties(self, amount, lhv, co2_factor, biogenic_share):
        """
        ...
        return fossil_co2, non_fossil_co2, final_lhv

    def update_dataset(self, dataset, supplier_key, amount):
        """
    ...
    def generate_regional_fuel_market(
        ...
        for prod_var in prod_vars:
            ...
            for supplier_key, supplier_val in possible_suppliers.items():
                ...
                fossil_co2, non_fossil_co2, final_lhv = self.calculate_fuel_properties(
                    amount, lhv, co2_factor, biogenic_co2_share
                )

                dataset = self.update_dataset(dataset, supplier_key, amount)
    ...
    def generate_fuel_markets(self):
        ...
        for fuel, activity in fuel_markets.items():
            if [i for e in self.fuel_labels for i in vars_map[fuel] if i in e]:
                ...
                # add to log
                for new_dataset in list(d_act.values()):
                    self.write_log(new_dataset)
