...
from .utils import DATA_DIR, get_crops_properties, write_log
        ...
        * Petitpas G. Boil-off losses along the LH2 pathway. US Dep Energy Off Sci Tech Inf 2018.
        ...
        for hydrogen_type, hydrogen_activity in hydrogen_sources.items():
            ...
            for datasets in list(new_ds.values()):
                write_log("fuel", "created", datasets, self.model, self.scenario, self.year)
                ...
                for vehicle, config in supply_chain_scenarios.items():
                    ...
                    for state in config["state"]:
                        ...
                        for distance in config["distance"]:
                            ...
                            # add to log
                            write_log("fuel", "created", [dataset], self.model, self.scenario, self.year)
        ...
        for climate in ["tropical", "temperate"]:
            ...
            for crop_type in climate_to_crop_type[climate]:
                ...
                for activity in activities:
                    ...
                    # add to log
                    for datasets in list(new_datasets.values()):
                        write_log("fuel", "created", datasets, self.model, self.scenario, self.year)
    ...
    def generate_regional_fuel_market(
        ...
        for prod_var in prod_vars:
            ...
            if not possible_suppliers:
                print(
                    f"No suppliers found for {prod_var} in {region} for dataset in location {dataset['location']}"
                )
    ...
    def generate_fuel_markets(self):
        ...
        for fuel, activity in fuel_markets.items():
            if [i for e in self.fuel_labels for i in vars_map[fuel] if i in e]:
                ...
                # add to log
                for datasets in list(d_act.values()):
                    write_log("fuel", "created", datasets, self.model, self.scenario, self.year)
