...
class IAMDataCollection:
    ...
    def __init__(
        ...
        for filepath in [
            ...
            prod_vars = self.__get_iam_variable_labels(filepath, variable="iam_aliases")
            eff_vars = self.__get_iam_variable_labels(filepath, variable="eff_aliases")
            energy_vars = self.__get_iam_variable_labels(
                filepath, variable="energy_use_aliases"
            )
            iam_vars.append(list(prod_vars.values()))
            iam_vars.append(list(eff_vars.values()))
            iam_vars.append(list(energy_vars.values()))

            if filepath == CROPS_PROPERTIES:
                crop_vars = self.__get_iam_variable_labels(
                    filepath, variable="land_use"
                )
                iam_vars.append(list(crop_vars.values()))
                crop_vars = self.__get_iam_variable_labels(
                    filepath, variable="land_use_change"
                )
                iam_vars.append(list(crop_vars.values()))

        iam_vars = [v for vs in iam_vars for v in vs]
        for v in iam_vars:
        ...
        data = self.__get_iam_data(
            key=key, filepath=filepath_iam_files, variables=iam_vars
        )
        ...
        cars = get_vehicle_fleet_composition(self.model, vehicle_type="car")
        if cars is not None:
            cars = cars.assign_coords(
                scenario=create_scenario_label(self.model, self.pathway, self.year)
            )
            cars = cars.expand_dims("scenario")
        self.trsp_cars = cars

        trucks = get_vehicle_fleet_composition(self.model, vehicle_type="truck")
        if trucks is not None:
            trucks = trucks.assign_coords(
                scenario=create_scenario_label(self.model, self.pathway, self.year)
            )
            trucks = trucks.expand_dims("scenario")
        self.trsp_trucks = trucks

        buses = get_vehicle_fleet_composition(self.model, vehicle_type="bus")
        if buses is not None:
            buses = buses.assign_coords(
                scenario=create_scenario_label(self.model, self.pathway, self.year)
            )
            buses = buses.expand_dims("scenario")
        self.trsp_buses = buses

    def __get_iam_variable_labels(self, filepath: Path, variable: str):
        """
    ...
    def __get_iam_electricity_markets(self, data):
        """
    ...
    def __get_iam_electricity_efficiencies(self, data):
        """
    ...
    def __get_iam_fuel_efficiencies(self, data: xr.DataArray) -> xr.DataArray:
        ...
        data_new.coords["variables"] = list(labels.keys())
    ...
    def __get_iam_production_volumes(self, dict_products, data) -> xr.DataArray:
        ...
        data_exp.coords["variables"] = list(dict_products.keys())
