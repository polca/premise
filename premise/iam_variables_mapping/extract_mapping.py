        ...
        if isinstance(value, dict):
            ...
            if 'energy_use_aliases' in value:
                for model, variable in value['energy_use_aliases'].items():
                    append_data(f"{main_key} (energy use)", model, variable, 'Specific Energy Use')
            if 'eff_aliases' in value:
                for model, variable in value['eff_aliases'].items():
                    append_data(f"{main_key} (efficiency)", model, variable, 'Efficiency')
            if 'electricity_use_aliases' in value:
                for model, variable in value['electricity_use_aliases'].items():
                    append_data(f"{main_key} (electricity use)", model, variable, 'Electricity Use')
                ...
                if isinstance(sub_value, dict):
                    if 'iam_aliases' in sub_value:
                        for model, variable in sub_value['iam_aliases'].items():
                            append_data(f"{main_key} - {sub_key}", model, variable, 'Production volume')
                    if 'energy_use_aliases' in sub_value:
                        for model, variable in sub_value['energy_use_aliases'].items():
                            append_data(f"{main_key} - {sub_key} (energy use)", model, variable, 'Specific Energy Use')
                    if 'eff_aliases' in sub_value:
                        for model, variable in sub_value['eff_aliases'].items():
                            append_data(f"{main_key} - {sub_key} (efficiency)", model, variable, 'Efficiency')
                    if 'electricity_use_aliases' in sub_value:
                        for model, variable in sub_value['electricity_use_aliases'].items():
                            append_data(f"{main_key} - {sub_key} (electricity use)", model, variable, 'Electricity Use')
                    if 'heat_use_aliases' in sub_value:
                        for model, variable in sub_value['heat_use_aliases'].items():
                            append_data(f"{main_key} - {sub_key} (heat use)", model, variable, 'Heat Use')
                    if "land use" in sub_value:
                        append_data(f"{main_key} - {sub_key}", 'IAM', sub_value['land use'], 'Land Use')
                    if "land_use_change" in sub_value:
                        append_data(f"{main_key} - {sub_key}", 'IAM', sub_value['land_use_change'], 'Land Use Change')
    df = pd.DataFrame(df_data, columns=['Key', 'IAM Model', 'Variable', 'Variable Type'])
    return df


# Process all *.yaml files except "constants.yaml"
for file in glob.glob('*.yaml'):
    if file in ['constants.yaml', 'missing_geography_equivalences.yaml', 'gains_regions_mapping.yaml', 'iam_region_to_climate.yaml']:
        continue
    file_name = file.split('/')[-1].replace('_variables.yaml', '').replace('_', ' ').title()
...
def create_refined_pivot_v2(df):
    ...
    df['Variable Category'] = df['Variable Type'] + ' ' + df['Energy use'] + df['Efficiency']
    ...
    pivot_df.columns = [col[0] + ' | ' + col[1].strip(' |') if col[1] else col[0] for col in pivot_df.columns]
