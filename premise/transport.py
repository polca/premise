        ...
        if "old" in self.mapping[self.vehicle_type]:
            for dataset in ws.get_many(
                ...
                for exc in ws.technosphere(
                    ...
                    # Check if the model exists in the mapping for this activity
                    if self.model in self.mapping[self.vehicle_type]["old"][exc["name"]]:
                        ...
                        # If model not found, skip this relinking (keep original reference)
                        logger.warning(
                            f"No {self.model} mapping found for '{exc['name']}' in {self.vehicle_type} transport. "
