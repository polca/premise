...
for sc in scenarios:
	...
	excel_files = [f for f in os.listdir(os.path.join('..', 'output', sc)) if (f.endswith('.xlsx') and not re.search(sc, f))]
	...
	# output
	out_df.to_excel(os.path.join('..', 'output', sc, f'gcam_{sc}.xlsx'), index=False)
