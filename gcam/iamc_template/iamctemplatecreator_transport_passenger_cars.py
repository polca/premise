...
def run_passenger_cars(scenario_name):
    ...
    output_df = output_df[output_df["subsector"].str.contains("Car")]
    input_df = input_df[input_df["mode"].str.contains("Car")]
    ...
    input_df = format_for_iamc(
        ...
        scenario_name,
        "EJ/yr",
        "Final Energy|Transport|Pass|Road|LDV|" + input_df["mode"] + "|" + input_df["technology"],
    )
    output_df = format_for_iamc(
        output_df,
        scenario_name,
        "million pkm/yr",
        "Distance|Transport|Pass|Road|LDV|" + output_df["subsector"] + "|" + output_df["technology"],
    )

    combine_and_write(
        [pivot_iamc(input_df), pivot_iamc(output_df)],
        scenario_name,
        "iamc_template_gcam_transport_passenger_cars.xlsx",
    )
